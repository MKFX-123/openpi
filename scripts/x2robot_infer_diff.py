...
class Args:
    ...
    weight_cls_ckpt: str = "runs/weight_classifier_diff/best_model.pt"
...
class _WeightMLP(nn.Module):
    """Binary weight classifier on diff features: 0=heavy, 1=light. Input: [B, W_in, 7]."""
    def __init__(self, W_in: int = 10, feature_dim: int = 7, hidden_dim: int = 256):
        super().__init__()
...
def main(args: Args) -> None:
    ...
    # Load diff binary weight classifier (7-dim, 0=heavy / 1=light)
    ...
    wc_model = _WeightMLP(
        W_in=_wc_W_in, feature_dim=7, hidden_dim=int(_wc_args.get("hidden_dim", 256))
    )
    wc_model.load_state_dict(_wc_ckpt["model_state"])
    wc_model.eval()
    wc_mean = np.asarray(_wc_ckpt["mean"], dtype=np.float32)  # (7,)
    wc_std  = np.asarray(_wc_ckpt["std"],  dtype=np.float32)  # (7,)
    logging.info(f"Weight classifier (diff): W_in={_wc_W_in}, feature_dim=7")
            ...
            while True:
                ...
                if args.policy_mode == "smw2smw":
                    ...
                    if vla_holding and not wc_prev_holding:
                        # Rising edge: grasp detected — run diff classifier once and hard-latch
                        # diff = master_right - follow_right, shape: (T, 7)
                        _wc_diff = (
                            master_state[:args.state_history_size + 1, 7:14]
                            - slave_state[:args.state_history_size + 1, 7:14]
                        ).astype(np.float32)
                        _win = _wc_diff[:_wc_W_in]  # (W_in, 7)
