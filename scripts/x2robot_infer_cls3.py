...
class Args:
    ...
    weight_cls_ckpt: str = "runs/weight_classifier_v2/best_model.pt"
...
def main(args: Args) -> None:
    ...
    logging.info(f"Weight classifier: W_in={_wc_W_in}, n_out={_wc_n_out}")
    ...
    while True:
        ...
        try:
            while True:
                ...
                if wc_locked_class == 0 and _raw_pred in (1, 2):
                    # Self-detected rising edge: latch immediately
                    wc_locked_class = _raw_pred
                    logging.info(
                        f"[WC] Grasp self-detected → {'heavy' if _raw_pred == 1 else 'light'}"
